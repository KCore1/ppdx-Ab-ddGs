...
def pearson_random_sampling(replica=1000):
    ...
    seeds1, seeds2 = [random.randrange(100000) for i in range(replica)], [
        random.randrange(100000) for i in range(replica)
    ]
    rp = np.array([get_rp(*fit(X, y, seeds1[i], seeds2[i])) for i in range(replica)])
    avg = np.average(rp)
    std = np.std(rp)
    plt.hist(
        rp, bins="auto", label="Average: %.2f\nStandard deviation: %.2f" % (avg, std)
    )
...
def bootstrap(replica=10000):
    ...
    nsamples = len(y_test)
    rp = [
        scipy.stats.pearsonr(y_test[idx], y_predict[idx])[0]
        for idx in np.random.choice(nsamples, size=(replica, nsamples), replace=True)
    ]
    avg = np.mean(rp)
    std = np.std(rp)
    ...
    plt.title("Correlation Error From Bootstrapping")
    plt.xlabel("Pearson Correlation Coefficient ($R_P$)")
    plt.ylabel("Count")
    plt.legend()
...
# rp, seeds1, seeds2 = pearson_random_sampling()
# print(seeds1[np.argmax(rp)], seeds2[np.argmax(rp)])
bootstrap()
