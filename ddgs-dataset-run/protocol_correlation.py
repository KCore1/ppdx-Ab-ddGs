...
def plot_crosscorr(mtx, nnamesx, nnamesy, cmap="Spectral", fname=None):
    ...
    cax = ax.matshow(
        mtx, interpolation="nearest", cmap=plt.get_cmap(cmap)
    )  # , vmin=-1, vmax=1)
    fig.colorbar(cax)
...
def main():
    ...
    comb = [
        ("modeller_fast", "modeller_veryfast"),
        ("modeller_fast", "modeller_slow"),
        ("modeller_fast", "rosetta"),
        ("modeller_veryfast", "modeller_slow"),
        ("modeller_veryfast", "rosetta"),
        ("modeller_slow", "rosetta"),
    ]
    ...
    plot_crosscorr(
        mtx_slope, cnames, newdesc, "bwr", "fig/protocol_correlation_slope.png"
    )
