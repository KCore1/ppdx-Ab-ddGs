...
def main():
    ...
    desclist = set(
        [
            ...
            for k in alldata[name][protocol].keys()
            if k.startswith(">TIME")
            and k != ">TIME_makemodel"
            and k != ">TIME_prodigy_ic_nis"
        ]
    )
    ...
    for desc in sorted(desclist):
        v = [
            np.average(list(data[protocol][desc].values()))
            for data in alldata.values()
            if desc in data[protocol].keys()
        ]
        descs.append(
            desc.replace(">TIME_", "")
            if desc != ">TIME_intermolecular_contacts"
            else "IC"
        )
    ...
    for protocol in [
        "modeller_fast"
    ]:  # 'modeller_veryfast', , 'modeller_slow', 'rosetta'
        timelst = list()
        ...
        print(
            "%-20s %10.3f %8.3f"
            % (protocol, np.average(timelst) / 60, np.std(timelst) / 60)
        )
